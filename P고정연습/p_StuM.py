...
def s_modi():
    ...
    if temp == 0 :
        ...
        if 1<=choice<=3 :
            print("현재 {} 점수: {}".format(title[choice+1], s[s_title[choice+1]]))
            s[s_title[choice+1]] = int(input("변경 {} 점수: ".format(title[choice+1])))
            s["total"] = s["kor"]+s["eng"]+s["math"]
            s["avg"] = s["total"]/3
            print("{} 학생의 {} 점수가 {}점으로 변경됨".format(name, title[choice+1], s[s_title[choice+1]]))
        else :
            print("1~3 중 입력")
    print()
...
def readStu():
    global s_no
    with open("c:/aaa/stu2.txt", "r", encoding="utf-8") as f1 :
        while True :
            line1 = f1.readline()
            line1 = line1.strip()
            if not line1 : break
...
# 학생 성적 파일 저장 함수 선언
def wrtStu():
    with open("c:/aaa/stu2.txt", "w", encoding="utf-8") as f1 :
        while True :
            for s in stuList :
